...
public class p1 {
	...
	public static void imprimirDatos(ArrayList<parser_archivos> archivos) throws IOException, TikaException, Exception {
		 ...
		 * "+--------------------------------------------+------------+-------------+--------+"
		 * ); System.out.
		 * println("|NOMBRE                                      |TIPO FICHERO|CODIFICACION | IDIOMA |"
		 * ); System.out.println(
		 * "+--------------------------------------------+------------+-------------+--------+"
		 * );
		 *
		 ...
		 *
		 * String act = arg + paths[i];
		 *
		 * // Crear objeto File actual = new File(act); // metadata InputStream is = new
		 * FileInputStream(actual); Metadata meta = new Metadata(); BodyContentHandler
		 * ch = new BodyContentHandler(); ParseContext parserContext = new
		 * ParseContext(); AutoDetectParser parser = new AutoDetectParser();
		 *
		 * // AutoDetectReader detector = new AutoDetectReader(is);
		 *
		 * parser.parse(is, ch, meta, parserContext); // language String text = new
		 * String(); text = tika.parseToString(actual); LanguageDetector identifier =
		 * new OptimaizeLangDetector().loadModels(); LanguageResult resultado =
		 * identifier.detect(text);
		 *
		 * // System.out.println(paths[i]); System.out .println("+" + paths[i] + "|" +
		 * tika.detect(actual) + " | " + " | " + resultado.getLanguage()); System.out
		 * .println(
		 * "+---------------------------------------------+------------+-------------+--------+"
		 * ); }
		 */
	...
	public static void main(String[] args) throws TikaException, Exception {
		System.out.println("\nCargando datos....\n");
		/**
		 * Codigo funcional /////////////////////////////////////// String auxi = new
		 * String(); // Creaamos una instancia de Tika con la configuracion por defecto
		 * Tika tika = new Tika(); // Se parsean todos los ficheros pasados como
		 * argumento y se extrae el contenido for (String file : args) { File f = new
		 * File(file);
		 *
		 * String type = tika.detect(f); System.out.println(file +":"+type);
		 *
		 *
		 * String text = null; try { text = tika.parseToString(f); auxi = text; } catch
		 * (IOException | TikaException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } System.out.print(text); }
		 *
		 *
		 *
		 *
		 * LanguageDetector identifier = new OptimaizeLangDetector().loadModels();
		 * LanguageResult resultado = identifier.detect(auxi); System.out.println("El
		 * lenguaje es: " + resultado.getLanguage());
		 *
		 */
